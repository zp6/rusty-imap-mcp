...
fuzz_target!(|data: &[u8]| {
    ...
    //
    // Use the scrubber's own boundary detection via the test-util
    // re-export so the harness's view of "header bytes" matches exactly
    // what the scrubber operated on.
